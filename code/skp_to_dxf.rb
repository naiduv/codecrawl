...
def dxf_export_mesh_file
      ...
      if ss.empty?
         answer = UI.messagebox("何も選択されていません。ファイル全体をエクスポートしますか？", MB_YESNOCANCEL)
         if( answer == 6 )
...
def dxf_write_stl(face,tform)
  ...
  polygons.each do |polygon|
  if (polygon.length == 3)
    for j in 0..polygon.length-1 do
      $mesh_file.puts( "facet normal " + mesh.normal_at(polygon[j].abs).x.to_s + " " + mesh.normal_at(polygon[j].abs).y.to_s + " " + mesh.normal_at(polygon[j].abs).z.to_s)
...
def dxf_dxf_options_dialog
   options_list=["polyface mesh","polyline","triangular mesh","lines","stl"].join("|")
   prompts=["DXF出力オプション"]
   enums=[options_list]
...
def dxf_dxf_units_dialog
   ...
   units_list=["メートル","センチメートル","ミリメートル","インチ","フィート"].join("|")
   prompts=["出力する単位: "]
   enums=[units_list]
   values=[current_unit]
...
if( not file_loaded?("skp_to_dxf.rb") )
   ...
   UI.menu("Tools").add_item("DXF/STL出力") { dxf_export_mesh_file }
end
